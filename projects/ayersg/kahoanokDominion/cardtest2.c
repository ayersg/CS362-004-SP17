...
*	Gillian Ayers
*
*	Test for Smithy card in Dominion
*	Card Description:
*		"+3 Cards"
*
*	References:
...
int main() {
	...
	printf("\nTEST 1: +3 cards\n");
	printf("Player gets to draw 3 cards.\n");
	...
	for(int i=0; i < MAX_PLAYERS; i++) {
		int num = i+1;
		printf("\nPlayer %d\n", num);
		printf("Test Hand Count after playing Smithy = %d, Orignal Hand Count = %d\n", testG.handCount[i], G.handCount[i]);
		printf("Test Deck Count after playing Smithy = %d, Original Deck Count = %d\n", testG.deckCount[i], G.deckCount[i]);
		printf("Test Discard Count after playing Smithy= %d, Original Discard Count = %d\n", testG.discardCount[i], G.discardCount[i]);
		
		if(i == currPlayer){
			if(testG.handCount[i] != G.handCount[i] + newCards){
				printf("TEST FAIL: Hand counts didn't increment properly. Must increment by 3.\n");
				flag = 1;
			}
			if(testG.deckCount[i] != G.deckCount[i] - newCards){
				printf("TEST FAIL: Deck counts didn't decrement properly. 3 cards drawn from deck, must decrement by 3.\n");
				flag = 1;
      		}
