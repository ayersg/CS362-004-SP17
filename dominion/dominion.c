...
int Seahag(struct gameState *state){
  ...
  for (int i = 0; i < state->numPlayers; i++){
    if (i != currentPlayer){
      ...
      state->deckCount[i]++; //BUG
      state->discardCount[i]--; //BUG
...
int Remodel(int choice1, int choice2, struct gameState *state, int handPos){

  int currentPlayer = whoseTurn(state);

  int j = state->hand[currentPlayer][choice1];  //store card we will trash

  if ( (getCost(state->hand[currentPlayer][choice1]) + 2) < getCost(choice2) )  //BUG
    {
      return -1;
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    /*********ADVENTURER***********/
    case adventurer:
      Adventurer(state);  //Refactored to its own function
    ...
    /*********REMODEL***********/			
    case remodel:
      Remodel(choice1, choice2, state, handPos);
